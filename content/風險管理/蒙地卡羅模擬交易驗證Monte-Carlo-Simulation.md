...
蒙地卡羅法假設每筆交易獨立，但真實市場中盈虧可能具有相依性：

- **正向相依**：贏了之後更容易贏（趨勢追蹤策略常見）→ 實際 MDD 比蒙地卡羅估算更大
- **反向相依**：贏了之後更容易輸（均值回歸策略常見）→ 實際 MDD 比蒙地卡羅估算更小

**連檢定判斷蒙地卡羅可信度**：

| Z 分數 | 含義 | 蒙地卡羅適用性 |
|--------|------|------------------|
| -1.96 ~ +1.96 | 獨立測試 | ✅ 適用，結果可信 |
| Z < -1.96 | 高度正向相依 | ⚠️ MDD 估算偏低，需保守解讀 |
| Z > +1.96 | 高度反向相依 | ⚠️ MDD 估算偏高，結果偏保守 |

### 95% 容忍度判斷

從蒙地卡羅模擬的 MDD 分布中，取 95% 累積機率對應的最大回落作為「必須容忍的範圍」：
- 超過此範圍 → 視為異常狀態，應停止策略或降低部位
- 以此設定保證金和資金準備
